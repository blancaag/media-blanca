...
def plot_ims(data, alpha=True, cmap=None): # plot_images(12)
    """
...
#     print('Here', len(data.shape[1:]), data.shape[1:])
    ...
    for i, ax in enumerate(axes.flat):
        ...
        elif (data[i].shape[2] == 4) & alpha: im = ax.imshow(data[i][:,:,[2,1,0,3]], cmap=cmap)
...
def plot_im(data, alpha=True, cmap=None): # plot_images(12)
    ...
    elif (data.shape[2] == 4) & alpha: im = ax.imshow(data[:,:,[2,1,0,3]], cmap=cmap)
