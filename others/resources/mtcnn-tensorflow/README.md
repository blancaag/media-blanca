## Changes I've made to the origin project by AITTSMD

- This can be installed as a package. Run `pip install .`. In your
  code you can now do `from mtcnn import detect_faces`.  
  WARNING: it is likely that this could work only for Unix systems. To make it
  work in Windows, modify the `setup.py` file and update the `dependencies`
  variable.
- Cleaned some files to respect more PEP8 rules.
